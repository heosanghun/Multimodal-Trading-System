...
class OllamaSentimentAnalyzer:
    def __init__(self, config=None):
        ...
        # DeepSeek 모델명 - 더 일반적인 모델로 기본값 설정
        ...
        # 프롬프트 템플릿 개선 - 암호화폐 뉴스 분석에 특화된 지시와 다양한 예시 추가
        self.prompt_template = """You are an expert cryptocurrency sentiment analyzer. Your task is to analyze the following crypto news text and determine whether it is bullish (positive for crypto prices), bearish (negative for crypto prices), or neutral.

News: {news}

Provide ONLY a JSON object with exactly these two fields:
1. "sentiment": One of these three values: "bullish", "bearish", or "neutral"
2. "score": A float value between -1.0 and 1.0, where:
   * -1.0 to -0.3: Strongly bearish
   * -0.3 to -0.1: Slightly bearish
   * -0.1 to 0.1: Neutral
   * 0.1 to 0.3: Slightly bullish
   * 0.3 to 1.0: Strongly bullish

Examples of bearish news:
- "Bitcoin Crashes Below $30,000 as Regulatory Concerns Mount" → {"sentiment": "bearish", "score": -0.8}
- "Traders Liquidate Positions Amid Market Uncertainty" → {"sentiment": "bearish", "score": -0.6}
- "Technical Analysis Shows Potential Downward Trend for Ethereum" → {"sentiment": "bearish", "score": -0.4}
- "Investors Remain Cautious as Bitcoin Struggles to Break Resistance" → {"sentiment": "bearish", "score": -0.25}

Examples of bullish news:
- "Bitcoin Hits All-Time High as Institutional Adoption Soars" → {"sentiment": "bullish", "score": 0.9}
- "ETFs Approval Expected to Bring Billions into Crypto Market" → {"sentiment": "bullish", "score": 0.7}
- "Market Analysis: Crypto Assets Show Bullish Patterns" → {"sentiment": "bullish", "score": 0.5}
- "Slight Increase in Trading Volume Suggests Growing Interest" → {"sentiment": "bullish", "score": 0.2}

Examples of neutral news:
- "Cryptocurrency Exchange Announces Platform Updates" → {"sentiment": "neutral", "score": 0.0}
- "Bitcoin Continues Sideways Trading in Narrow Range" → {"sentiment": "neutral", "score": 0.0}

IMPORTANT: Respond ONLY with a valid JSON object. No explanations, text, or additional content.
Your response must parse as valid JSON and contain only the "sentiment" and "score" fields.
"""
    ...
    def call_ollama_api(self, prompt):
        ...
        try:
            ...
            data = {
                ...
                "stream": False,
                "options": {
                    "temperature": 0.05,  # 더 낮은 온도로 설정하여 확정적인 응답 유도
    ...
    def _extract_json_from_text(self, text):
        """
        텍스트에서 JSON 객체 추출 및 파싱 (개선된 파싱 로직)
        ...
        try:
            ...
            if json_start >= 0 and json_end >= 0 and json_end > json_start:
                ...
                try:
                    ...
                    # 속성명에 따옴표 추가
                    try:
                        # sentiment와 score 속성에 따옴표 추가
                        json_str = json_str.replace('sentiment', '"sentiment"').replace('score', '"score"')
                        result_json = json.loads(json_str)
                        if isinstance(result_json, dict) and 'sentiment' in result_json and 'score' in result_json:
                            return self._validate_sentiment_json(result_json)
                    except json.JSONDecodeError:
            ...
            # 3. 텍스트 분석 기반 대체 로직
            self.logger.warning("JSON 파싱 실패, 텍스트 기반 분석을 시도합니다.")
    ...
    def analyze_sentiment(self, news_data):
        ...
        try:
            ...
            # 배치 처리 로직
            for start_idx in tqdm(range(0, len(news_data), self.batch_size), desc="뉴스 감성 분석"):
                batch = news_data.iloc[start_idx:start_idx + self.batch_size]
                    ...
                    for retry_count in range(self.max_retries):
                        try:
                            ...
                            if retry_count == self.max_retries - 1:
                                ...
                                # 실패 시 백업 방법으로 텍스트 기반 분석
                                sentiment_result = self._analyze_text_fallback(text)
                            else:
                                self.logger.warning(f"API 호출 실패, 재시도 중: {str(e)}")
                                time.sleep(1 * (retry_count + 1))  # 지수 백오프
                ...
                # 배치 간 대기 (필요한 경우)
                if start_idx + self.batch_size < len(news_data):
            ...
            self.logger.info(f"Ollama 뉴스 감성 분석 결과: {overall_sentiment}, 점수: {sentiment_score:.2f}")
            self.logger.info(f"감성 분포: 강세 {bullish_ratio:.2%}, 약세 {bearish_ratio:.2%}, 중립 {neutral_ratio:.2%}")
            
            # 결과 저장
            if self.results_dir:
                json_path = os.path.join(self.results_dir, "ollama_sentiment_analysis_results.json")
                with open(json_path, 'w') as f:
                    json.dump(analysis_result, f, indent=4)
                self.logger.info(f"Ollama 감성 분석 결과 저장: {json_path}")
            
            return analysis_result
        
        except Exception as e:
            self.logger.error(f"Ollama 뉴스 감성 분석 오류: {str(e)}")
            return {
                'overall_sentiment': 'neutral',
                'sentiment_score': 0.0,
                'bullish_ratio': 0.0,
